...
namespace ETicaretAPI.Application.Features.Commands.ProductImageFile.ChangeShowcaseImage
{
    public class ChangeShowcaseImageCommandHandler : IRequestHandler<ChangeShowcaseImageCommandRequest, ChangeShowcaseImageCommandResponse>
    {
        ...
        {
            var querry = _productImageFileWriteRepository.Table
            ...
            var data = await querry.FirstOrDefaultAsync(p=>p.p.Id == Guid.Parse(request.ProductId) && p.pif.Showcase);
            
            if (data != null)
                data.pif.Showcase = false;

            var image = await querry.FirstOrDefaultAsync(p => p.pif.Id == Guid.Parse(request.ImageId));
